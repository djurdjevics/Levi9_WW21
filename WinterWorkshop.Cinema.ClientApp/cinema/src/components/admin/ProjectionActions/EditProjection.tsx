...
import { IMovie, IAuditorium, IProjection } from "../../../models";
import {getProjectionById, getAllAuditoriums, getCurrentMoviesAndProjections, editProjectionById, checkIfTicketExistForProjection } from "../../APICommunication";
...
const EditProjection: React.FC = (props: any) => {
  ...
  const validate = (id, value) => {
    if (id === "projectionTime") {
      if (!value) {
        setState((prevData) => ({...prevData, projectionTimeError: "Chose projection time",
        canSubmit: false}));
      } else {
      ...
      if (!value) {
        setState((prevData) => ({...prevData, movieIdError: "Please chose movie from dropdown",
        canSubmit: false}));
      } else {
      ...
      if (!value) {
        setState((prevData) => ({...prevData, auditoriumIdError: "Please chose auditorium from dropdown",
        canSubmit: false}));
      } else {
  ...
  const editProjection = (projectionId: string) => {
    ...
    checkIfTicketExistForProjection(projectionId).then((dataFromAPI)=>{
      if(dataFromAPI)
      {
        if(dataFromAPI.length === 0)
        {
          editProjectionById(data, projectionId).then((respon)=>{
            if(respon.id)
            {
        ...
        {
          NotificationManager.error("Projection cannot be changed, because there are purchased tickets.");
          props.history.push('/dashboard/AllProjections');
        }
      }
    })
  ...
  return (
    <Container>
      <Row>
        <Col>
          <h1 className="form-header projections-title">Edit Projection</h1>
